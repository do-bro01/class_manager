...
export async function createSession(
  ...
  try {
    ...
    if (!session_date) return { success: false, error: '회차 날짜를 선택해주세요.' }
...
export async function markAttendance(
  ...
  try {
    ...
    const deletions = marks.filter((m) => m.status === null).map((m) => m.user_id)
...
export async function selfCheckIn(sessionId: string): Promise<ActionResult> {
  ...
  if (!session.check_in_open) {
    return { success: false, error: '체크인이 닫혀 있습니다. 강사에게 문의하세요.' }
  }
...
export async function getSessionRoster(
  ...
  const recordMap = new Map(
    (records ?? []).map((r: Record<string, unknown>) => [
      r.user_id as string,
      { status: r.status as AttendanceStatus, marked_at: r.marked_at as string },
    ])
  )

  const rows: SessionRosterRow[] = (enrollments ?? []).map((e: Record<string, unknown>) => {
