...
import { getSessionsWithStats, getStudentAttendance } from '@/lib/actions/attendance'
...
export default async function ClassDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [enrollments, notices, qnas, attendanceSessions, studentAttendance] =
    await Promise.all([
      ...
      getNotices(id),
      getQnAs(id),
      isInstructor ? getSessionsWithStats(id) : Promise.resolve([]),
      !isInstructor ? getStudentAttendance(id) : Promise.resolve(null),
    ])
        ...
        <Tabs defaultValue="notices">
          <TabsList>
            ...
            <TabsTrigger value="attendance">
              출결{isInstructor ? ` (${attendanceSessions.length})` : ''}
            </TabsTrigger>
            {isInstructor && (
          ...
          <TabsContent value="attendance" className="mt-6">
            {isInstructor ? (
              <AttendanceSessionList classId={id} sessions={attendanceSessions} />
            ) : (
              studentAttendance && <StudentAttendanceView summary={studentAttendance} />
            )}
          </TabsContent>
